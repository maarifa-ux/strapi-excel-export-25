...
module.exports = ({ strapi }) => ({
  async getDropDownData(ctx) {
    ...
    const filteredTypes = Object.keys(contentTypes).filter((key) => {
      const model = contentTypes[key];
      return model.kind === 'collectionType' || model.kind === 'singleType';
    });

    // Döngü ile türleri işleyin
    for (let i = 0; i < filteredTypes.length; i++) {
      const key = filteredTypes[i];
      const model = contentTypes[key];
      
      if (model?.kind === "collectionType") {
        const checkExist = array?.find(c => c === model.uid);
        if (checkExist) {
          dropDownValues.push({
            label: model?.info?.displayName,
            value: model?.uid,
          });
        }
      }
